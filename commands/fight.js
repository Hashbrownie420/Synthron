...
function loadStats() {
    try {
        if (!fs.existsSync(statsFile)) {
            // Wenn die Datei nicht existiert, erstelle eine leere Datei
...
module.exports = {
    ...
    execute: async (sock, sender, args, msg) => {
        ...
        if (p1Total > p2Total) {
            ...
            stats[player1].points += pointsForRoundPlayer1;  // Punkte des Siegers hinzufügen
            stats[player2].points += pointsForRoundPlayer2;  // Punkte des Verlierers hinzufügen
            ...
            stats[player2].points += pointsForRoundPlayer2;  // Punkte des Siegers hinzufügen
            stats[player1].points += pointsForRoundPlayer1;  // Punkte des Verlierers hinzufügen
        } else {
            winner = null; // Unentschieden
            pointsForRoundPlayer1 = pointsDraw;
            pointsForRoundPlayer2 = pointsDraw;
            stats[player1].points += pointsForRoundPlayer1; // Punkte für Unentschieden
            stats[player2].points += pointsForRoundPlayer2; // Punkte für Unentschieden
        }

        stats[player1].fights++;
        stats[player2].fights++;
        saveStats(stats);  // Speichern der Stats nach dem Kampf
...
📈 *Stats:*
- @${player1.split('@')[0]} \n${stats[player1].wins} Siege | ${stats[player1].fights} Kämpfe | Punkte: ${stats[player1].points} \n(Erhielt ${pointsForRoundPlayer1} Punkte in dieser Runde)\n
- @${player2.split('@')[0]} \n${stats[player2].wins} Siege | ${stats[player2].fights} Kämpfe | Punkte: ${stats[player2].points} \n(Erhielt ${pointsForRoundPlayer2} Punkte in dieser Runde)
`;
